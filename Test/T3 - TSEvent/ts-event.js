			...
			prototype: {
				...
				init: function(t1, t2){
					...
					aCoord.forEach( function(coord){
						...
						c[coord+'Ray'] = Math.sqrt( Math.pow( t1[coord+'X'] - t2[coord+'X'], 2) * Math.pow( t1[coord+'X'] - t2[coord+'Y'], 2) ) / 2;
					} );

					y = t2.screenY - t1.screenY;
					this.angle = ( y > 0 ? 2 * Math.PI : 0) + Math.atan2(t2.screenX - t1.screenX, t2.screenY - t1.screenY);
		...
		TSManager,
		{
			...
			move: function(e){
				...
				fings.forEach( function(fing){
					...
					if( dx != 0 ){
						typ = dx > 0 ? 'swipeRight' : 'swipeLeft';
						oFingEvt[typ] = oFingEvt[typ] || [];
						oFingEvt[typ].push(fing);
					}
					if( dy != 0 ){
						typ = dy > 0 ? 'swipeBottom' : 'swipeTop';
						oFingEvt[typ] = oFingEvt[typ] || [];
						oFingEvt[typ].push(fing);
					}
				} );
				...
				if( bNewCirc ){
					...
					typ = c.screenRay > pc.screenRay ? 'spread' : 'pinch';
					oFingEvt[typ] = aFing;
					if( c.angle != pc.angle ){
